...
export const actions: Actions = {
  default: async ({ request }) => {
    ...
    try {
      const profile = await fetchGithubProfile(form.data.username, form.data.token);
      ...
      let prompt = `Berikan roasting singkat dengan kejam dan menyindir dalam bahasa gaul untuk profile github berikut: ${form.data.username}. Berikut detailnya: "${JSON.stringify(data)}"`;
      if (data.location != null && !data.location.includes("Indonesia")) {
        prompt = `Give a short and harsh roasting for the following github profile: ${form.data.username}. Here are the details: "${JSON.stringify(data)}"`;
      }
      ...
      return { form, type: "success", text: response };
    } catch (error) {
      console.error(error);

      if (error instanceof Error) {
        return { form, type: "error", text: error.message, status: 400 };
      }

      return { form, type: "error", text: String(error), status: 500 };
    }
  },
};
